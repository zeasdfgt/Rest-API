package ca.ulaval.glo4002.reservation.reports.rest.mappers;

import ca.ulaval.glo4002.reservation.reports.exceptions.InvalidReportTypeException;

import javax.servlet.http.HttpServletResponse;
import javax.ws.rs.core.Response;
import javax.ws.rs.ext.ExceptionMapper;
import javax.ws.rs.ext.Provider;
import java.util.Map;
import java.util.TreeMap;

@Provider
public class InvalidReportTypeExceptionMapper
    implements ExceptionMapper<InvalidReportTypeException> {
  private final int HTTP_STATUS_CODE = HttpServletResponse.SC_BAD_REQUEST;
  private final String ERROR_TYPE = "INVALID_TYPE";
  private final String DESCRIPTION_MESSAGE =
      "Type must be either total or unit and must be specified.";

  @Override
  public Response toResponse(InvalidReportTypeException invalidReportTypeException) {
    Map<String, String> responseBody = new TreeMap<>();
    responseBody.put("error", ERROR_TYPE);
    responseBody.put("description", DESCRIPTION_MESSAGE);

    return Response.status(HTTP_STATUS_CODE).entity(responseBody).build();
  }
}
